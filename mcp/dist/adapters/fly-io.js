...
export const flyIoAdapter = {
    ...
    async check() {
        ...
        try {
            const response = await fetch(`https://api.machines.dev/v1/apps/${encodeURIComponent(appName)}/machines`, {
                method: "GET",
                headers: {
            ...
            return {
                target: `fly:${appName}`,
                status: running.length === total && total > 0
                    ? "healthy"
                    : running.length > 0
                        ? "degraded"
                        : "down",
